...
def validate_prompts(value: object, *, expected_plugin_key: str) -> dict[str, Any]:
    ...
    for item in value["items"]:
        if not isinstance(item, dict) or set(item) != {"id", "scenario", "content", "createdAt"}:
        ...
        seen.add(prompt_id)
        _require_text(item["scenario"], "常用场景", maximum=200, single_line=True)
        _require_text(item["content"], "Prompt 内容", maximum=100_000)
        _require_timestamp(item["createdAt"])
    return value
...
def _require_timestamp(value: object) -> str:
    if not isinstance(value, str) or not value.endswith("Z") or len(value) > 40:
        raise PromptValidationError("Prompt 添加时间无效")
    try:
        datetime.fromisoformat(value[:-1] + "+00:00")
    except ValueError:
        raise PromptValidationError("Prompt 添加时间无效") from None
    return value
