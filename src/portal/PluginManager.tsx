...
export function PluginManager({
  ...
  const configFor = (root: string): PluginImportConfig => ({
    pluginRoot: root.trim(),
    target: target.trim(),
  ...
  const preview = async (root = pluginRoot) => {
    try {
      setBusy(true);
      setError("");
      setCandidate(await client.previewImport(configFor(root)));
    } catch (reason) {
  ...
  const selectDirectory = async () => {
    try {
      ...
      setCandidate(await client.previewImport(configFor(selection.path)));
    } catch (reason) {
      setCandidate(undefined);
    ...
    <section className="plugin-manager" aria-label="插件管理">
      ...
      <div className="edit-form">
        ...
        <details className="advanced-import-options">
          ...
          <button disabled={busy || !pluginRoot.trim()} onClick={() => preview()} type="button">重新生成预览</button>
        </details>
